    ...
    <div className={`min-h-screen ${shellClass}`}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {layoutKey === 'article-editorial' || layoutKey === 'article-journal' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
            <div className={`rounded-[2rem] p-8 ${ui.panel}`}>
              <h1 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-foreground">
                {taskConfig?.description || 'Curated article content.'}
              </h1>
              <p className={`mt-4 max-w-3xl text-sm leading-7 ${ui.muted}`}>
                Explore a clean archive of articles with category-based browsing. Read by topic, discover relevant stories faster, and keep focus on content.
              </p>
            </div>

            <div className={`rounded-[2rem] p-6 ${ui.soft}`}>
              <p className={`text-xs uppercase tracking-[0.2em] ${ui.muted}`}>Category Filter</p>
